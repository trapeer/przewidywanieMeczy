...
void wczytaj_wartosci(float a[40])
{
	FILE *fp;
	fp = fopen("zmienne.txt", "r");
	if (fp == NULL)
	{
	...
	for(i=0;i<40;i++)
	{
		...
		if(i<10)
		{
			for(k=0;k<7;k++) fscanf(fp,"%c",&x);
		}
		else
		{
			for(k=0;k<8;k++) fscanf(fp,"%c",&x);
		}
		fscanf(fp,"%f",&a[i]);
	}
	fclose(fp);
...
main()
{
	...
	printf("\n\ndruga partia meczy do sprawdzenia:\n\n");
